...
class Args:
    ...
    env_id: str = "AirbotPlayPickCube"  # Set the environment name here
    total_timesteps: int = 1000000000  # Adjust based on task complexity
    learning_rate: float = 3e-4  # Good starting point for most tasks
    num_envs: int = 2048  # Increase the number of parallel environments for speed
    num_eval_envs: int = 4

    num_steps: int = 50  # Number of steps per rollout
    num_eval_steps: int = 50

    anneal_lr: bool = False  # Not enabling learning rate annealing
    gamma: float = 0.99  # Standard for PPO
    gae_lambda: float = 0.95  # Generalized Advantage Estimation
    num_minibatches: int = 32  # Standard value for minibatch size
    update_epochs: int = 4  # Number of epochs per update
    norm_adv: bool = True  # Normalize advantages
    clip_coef: float = 0.2  # Standard for clipping in PPO
    clip_vloss: bool = False
    ent_coef: float = 0.0  # Entropy cost to encourage exploration
    vf_coef: float = 0.5  # Value function coefficient
    max_grad_norm: float = 0.5
    target_kl: Optional[float] = 0.1
    reward_scale: float = 1.0  # Default scaling
    eval_freq: int = 25  # Frequency of evaluations

    # Runtime filled
    batch_size: int = 0  # Computed at runtime
    minibatch_size: int = 0  # Computed at runtime
    num_iterations: int = 0  # Computed at runtime
