...
class MJXManiLikeVectorEnv:
    ...
    def __init__(
        ...
        gs_camera_id: int = 0,
        gs_camera_ids: Optional[Sequence[int]] = None,
        gs_camera_names: Optional[Sequence[str]] = None,
        gs_height: int = 60,
        gs_width: int = 80,
        ...
        self._gs_camera_ids = [int(x) for x in gs_camera_ids] if gs_camera_ids is not None else None
        self._gs_camera_names = [str(x) for x in gs_camera_names] if gs_camera_names is not None else None
        ...
        self._camera_ids = self._resolve_camera_ids()

        # vision spec
        num_cams = len(self._camera_ids)
        self._vision_spec = MJXVisionSpec(rgb_shape=(self._gs_height, self._gs_width, 3 * num_cams))
    ...
    def _resolve_camera_ids(self) -> List[int]:
        if self._gs_camera_names is not None:
            if self.mj_model is None:
                raise RuntimeError("Camera names require mj_model to resolve camera IDs.")
            camera_ids = []
            for name in self._gs_camera_names:
                try:
                    camera_ids.append(int(self.mj_model.camera(name).id))
                except Exception as exc:
                    raise ValueError(f"Unknown camera name '{name}'") from exc
        else:
            camera_ids = self._gs_camera_ids if self._gs_camera_ids is not None else [self._gs_camera_id]
        if not camera_ids:
            raise ValueError("At least one camera id must be specified for rendering.")
        if self.mj_model is not None:
            ncam = int(self.mj_model.ncam)
            for cam_id in camera_ids:
                if cam_id < 0 or cam_id >= ncam:
                    raise ValueError(f"Camera id {cam_id} out of range (ncam={ncam}).")
        return camera_ids
    ...
    def _render_gs_rgb(self, state) -> torch.Tensor:
        ...
        if cam_xmat.ndim == 3:  # (B,Ncam,9)
            cam_xmat = cam_xmat.view(cam_xmat.shape[0], cam_xmat.shape[1], 3, 3)
        ...
        # choose one or more cameras for PPO
        cam_ids = self._camera_ids
        if len(cam_ids) == 1:
            rgb = rgb[:, cam_ids[0], ...]  # (B,H,W,3)
        else:
            rgb = rgb[:, cam_ids, ...]  # (B,Ncam,H,W,3)
            rgb = rgb.permute(0, 2, 3, 1, 4).reshape(
                rgb.shape[0], self._gs_height, self._gs_width, 3 * len(cam_ids)
            )
        ...
        if self.vision_backend == "gs":
            ...
            if rgb.ndim == 5:
                ...
                if len(cam_ids) == 1:
                    ...
                    rgb = rgb[:, cam_ids, ...]
                    rgb = rgb.permute(0, 2, 3, 1, 4).reshape(
                        rgb.shape[0], rgb.shape[2], rgb.shape[3], 3 * len(cam_ids)
                    )
            obs["rgb"] = rgb
